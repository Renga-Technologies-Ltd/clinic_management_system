...
const MapChart = ({ setTooltipContent, data, mapSource, mapType }) => {
  	return (
		<ComposableMap style={{transform: `${mapType === 'kenya' ? 'translateY(20px)' : 'none'}`}} data-tip="" height={380} projectionConfig={{ scale: 145 }}>
			<Geographies geography={mapSource}>
				{({ geographies }) =>
					geographies.map(geo => {
						const geoName = mapType === 'kenya' ? geo.properties.name : geo.properties.NAME_1 
						return (
							<Geography
								key={geo.rsmKey}
								geography={geo}
